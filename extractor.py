...
# Canonical competitor taxonomy. A logo appearing on a positioning chart (e.g. "AWS") is not
# automatically a direct competitor — it may be a platform the company is built on (platform
# threat), a company that could plausibly enter the space (adjacent player), or a company
# solving a related-but-different problem (indirect). Free-text model output gets normalized
# onto this set rather than validated strictly, so an unexpected label never crashes the pipeline.
COMPETITOR_TYPES = {"direct", "indirect", "platform_threat", "adjacent"}
THREAT_LEVELS = {"low", "medium", "high"}


def _normalize_competitor_type(raw: str | None) -> str:
    v = (raw or "").strip().lower().replace("-", "_").replace(" ", "_")
    if v in COMPETITOR_TYPES:
        return v
    if "platform" in v:
        return "platform_threat"
    if "indirect" in v:
        return "indirect"
    if "adjacent" in v or "potential" in v:
        return "adjacent"
    if "direct" in v:
        return "direct"
    # Unclear/unlabeled mentions default to "indirect" rather than "direct" — the model
    # naming a company (e.g. from a logo on a slide) is weaker evidence than an explicit
    # "our direct competitors are X, Y, Z" statement, so we don't overstate the threat.
    return "indirect"


def _normalize_threat_level(raw: str | None) -> str:
    v = (raw or "").strip().lower()
    return v if v in THREAT_LEVELS else "medium"
...
async def extract_company(slide_analyses: list[SlideAnalysis], file_hash: str, client: AsyncOpenAI) -> Company:
    ...
    cache_k = cache_key(file_hash, "company_v5", slides_hash)
    ...
    for field in ["competitors", "risks", "regulatory_risks", "benchmarks", "founders"]:
    ...
    if isinstance(data.get("competitors"), list):
        ...
        for c in data["competitors"]:
            if isinstance(c, str):
                fixed.append({"name": c, "competitor_type": "indirect"})
            else:
                c["competitor_type"] = _normalize_competitor_type(c.get("competitor_type"))
                c["threat_level"] = _normalize_threat_level(c.get("threat_level"))
                fixed.append(c)
    ...
    try:
        ...
        logger.error("Company extraction failed validation, falling back to minimal record: %s", exc)
        company = Company(name=data.get("name", "Unknown"), description=data.get("description", ""))

    company.metrics = calculate_derived_metrics(company.metrics)
    company.benchmarks = generate_benchmarks(company)

    logger.info(
        "Extracted '%s': %d founders, %d competitors, %d risks, %d benchmarks",
        company.name, len(company.founders), len(company.competitors), len(company.risks), len(company.benchmarks)
    )
...
def calculate_derived_metrics(metrics: Metrics) -> Metrics:
    ...
    if metrics.cac and metrics.revenue_per_customer and metrics.gross_margin and metrics.gross_margin > 0:
        ...
        if monthly_gp > 0:
            metrics.cac_payback_months = round(metrics.cac / monthly_gp, 1)
...
def run_sanity_checks(company: Company) -> list[SanityCheck]:
    ...
    if m.ltv_cac_ratio is not None and m.ltv_cac_ratio < 1.0:
        checks.append(SanityCheck(
            name="CAC exceeds LTV", severity="critical",
            message=f"LTV:CAC ratio is {m.ltv_cac_ratio:.1f}:1.",
            recommendation="Do not invest.", passed=False,
        ))

    if m.runway_months is not None and m.runway_months < 6:
        checks.append(SanityCheck(
            name="Runway < 6 months", severity="critical",
            message=f"Runway is {m.runway_months:.0f} months.", passed=False,
        ))

    if m.gross_margin is not None and m.gross_margin < 40:
        checks.append(SanityCheck(
            name="Low Gross Margin", severity="warning",
            message=f"Gross margin is {m.gross_margin:.1f}%.", passed=False,
        ))

    if m.churn_rate is not None and m.churn_rate > 5:
        checks.append(SanityCheck(
            name="High Churn", severity="critical" if m.churn_rate > 10 else "warning",
            message=f"Monthly churn is {m.churn_rate:.1f}%.", passed=False,
        ))

    if not checks:
        checks.append(SanityCheck(name="No red flags", severity="info", message="No deterministic red flags.", passed=True))
...
def calculate_traction_score(metrics: Metrics) -> float:
    score = 50.0
    if metrics.arr:
        if metrics.arr >= 10_000_000:
            score += 25
        elif metrics.arr >= 1_000_000:
            score += 15
